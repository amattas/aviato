...
- [process] The §11.3 pyproject-extras pin-form check flags self-referential extras (e.g. a dev extra listing `pkgname[cli]` to ride a sibling extra along) as unpinned, even when the referenced extra's contents are fully exact-pinned — live: pydmp had to inline duplicate pins with a keep-in-sync comment. Teach `unpinned_pyproject_extra_lines` to accept `name[extra]` when `name` equals the project's own name (the pins resolve within the same file the check already scans). — source: 2026-07-19 pydmp adoption · aviato/plugins/actionpins.py
