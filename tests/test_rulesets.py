...
def test_ruleset_with_profile_injects_language_verify_check() -> None:
    ...
    contexts = _status_contexts(render_all_rulesets(root=POLICY_DATA_ROOT, extra_status_checks=["ci / Python CI"]))
    assert "ci / Python CI" in contexts
    assert "common-lint / Common lint" in contexts
    # idempotent: a re-injected common check is not duplicated
    payloads = render_all_rulesets(root=POLICY_DATA_ROOT, extra_status_checks=["common-lint / Common lint"])
...
def test_required_approvals_zero_override_propagates() -> None:
    ...
    counts = _approval_counts(render_all_rulesets(root=POLICY_DATA_ROOT, required_approvals=0))
    assert counts
...
def _branch_and_tag() -> tuple[Ruleset, Ruleset]:
    ...
    rendered = render_all_rulesets(root=POLICY_DATA_ROOT, extra_status_checks=["ci / Python CI"])
